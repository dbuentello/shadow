...
*/
	...
	public static function phRETS() { echo __METHOD__; }
	...
	public function FetchRow($pointer_id) {
		...
		// Start: Application Callback
                $config = \Shadow\Config::get('rets.search_query_callback');
		if (!empty($config)) {
			call_user_func(array($config['object'], $config['method']));
		}
                // End: Application Callback
	...
	public function Connect($login_url, $username, $password, $ua_pwd = "") {
		...
		if (isset($this->last_request['ReplyCode']) && $this->last_request['ReplyCode'] == 0) {
			...
			if (isset($this->last_request['ReplyCode']) && isset($this->last_request['ReplyText'])) {
